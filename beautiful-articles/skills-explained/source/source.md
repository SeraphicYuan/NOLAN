...
- **Category:** Agents
- **Product:** Claude apps, Claude Platform
- **Date:** March 5, 2026
- **Reading time:** 5 min
...
**Example:** Create a code-reviewer subagent with access to Read, Grep, and Glob tools but not Write or Edit. When you modify code, Claude automatically delegates to this subagent for quality and security review without risking unintended code changes.
...
> [NOTE: The original article had a comparison table here that did not survive HTML extraction. Reconstructed from the article's own descriptions of each building block — see extraction-notes.md.]

| Building block | What it is | Persistence | When to reach for it |
|---|---|---|---|
| **Prompts** | Natural-language instructions in conversation | Ephemeral (single conversation) | One-off requests, immediate context, conversational refinement |
| **Skills** | Folders of instructions/scripts loaded on-demand | Persistent + portable across conversations | Repeatable procedures and specialized expertise ("how to do something") |
| **Projects** | Self-contained workspace with knowledge base | Persistent within the project | Background knowledge that should inform every conversation ("what to know") |
| **Subagents** | Independent assistants with own context + tools | Purpose-built, reusable config | Specialized, independent task execution with tool restriction + context isolation |
| **MCP** | Open protocol connecting Claude to external systems | Persistent live connection | Accessing external data and tools (connectivity) |
