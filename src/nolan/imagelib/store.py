...
class ImageLibrary:
    ...
    def enrich_pdia_details(self, *, limit: int = 500, workers: int = 4,
        ...
        * **THE HOLDING INSTITUTION.** PDIA is an aggregator — its images are re-hosted from the
        ...
        * **THE TWO RIGHTS CLAIMS**, verified rather than assumed. The site declares CC0
          ...
          per-item guarantee — the exact trap the Library of Congress probe flagged. A row whose
    ...
    def _enrich_pdia_chunk(self, rows, out: dict, *, workers: int, progress=None) -> None:
        ...
        for uuid, d in got.items():
            ...
            # a work is `pd-us` — public domain in the United States ONLY. Leaving those labelled
            ...
            if lic is None:
                ...
                if len(out["reasons"]) < 12:
                    ...
                    out["reasons"].append(f"pdia:{uuid}: unrecognised rights {codes} — "
        ...
        # `reindex_identity` only fills rows MISSING from the vector store — it would examine
