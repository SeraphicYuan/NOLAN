...
**NOLAN compose-first mode (default in this repo).** Dispatch `sub-agents/compose-first-frame-worker.md` per frame instead of `frame-worker.md`, passing everything above **plus `BRIDGE_DIR`** — the absolute path to the NOLAN composer (`render-service/_lab_hyperframes/bridge/`). The worker expresses each of the frame's Scenes with a reusable composer template (`catalog.json` — stat / statement / geo / media_ground / prop_cutout) and builds the frame deterministically via `author.py` (a validating gate), hand-authoring a bespoke `raw` scene only where no template fits. It emits the identical `compositions/frames/<frame_id>.html` artifact, so **everything else in Step 5 — duration sync, SFX, captions, `assemble-index.mjs` — is unchanged**. The stock `frame-worker.md` remains the fallback for a frame that is entirely bespoke. Rationale + the module contract: `BRIDGE_DIR/catalog.json` and the lab `REPORT.md`.
