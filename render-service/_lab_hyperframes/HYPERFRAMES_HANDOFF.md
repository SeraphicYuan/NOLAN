...
- **`bridge/vo_bridge.py`** — NOLAN cloned voiceover (`voice_pipeline`: voiceover.mp3 + `_work/sec_*.wav` or `segments/`) → the faceless `audio_meta.json` shape (`voices[{frame,path,duration_s,words}]`, 1:1 section→frame; wavs → `assets/voice/0N.wav`). Lets the EXISTING chain run on the cloned voice — `audio.mjs sync-durations` (frame dur = section dur; narration owns duration) + `assemble-index` (root voice track 10) + `hyperframes render` — with NO second TTS pass (replaces `audio.mjs generate`). Wired into `new_essay(voiceover=…)` + `_scaffold_preview` (narrated frame preview); on the /map Bridge tab; `tests/test_vo_bridge.py`.
