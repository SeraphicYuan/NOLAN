...
# ======================================================================================
# TIER-2 BLOCKS (merged from compose_extension.py, 2026-07-26)
#
# They lived in a sibling module until every gate written for "the composer" turned out to
# mean compose.py ONLY: check_reveal_sync.py and test_block_token_fidelity.py each read this
# file and no other, so 14 blocks sat outside the front-loading gate and the token-fidelity
# gate, and the capabilities then drifted (data.ground is a phantom on them, no _reveal_dur,
# partial cue support). That is WIRING_CHECKLIST pitfall #4 at file granularity. One file, one
# registry, every gate covers everything.
#
...
def _resolve_asset(subj: str):
    """Absolute path to a subject asset at compose time, or None. _ASSET_BASE (the comp dir) is
...
def bar_race(sid, sc):
    ...
    for k in range(ns):
        ...
        tl.append(f'tl.set("#{sid}-period",{{textContent:{json.dumps(str(steps[k]))}}},{t:.2f});')
        for si, s in enumerate(series):
            ...
            tl.append(f'tl.set("#{sid}-v{si}",{{textContent:{json.dumps(_num(v) if isinstance(v,float) else v)}}},{t:.2f});')
    return frag, tl
...
def slope(sid, sc):
    ...
    times = _reveal_times(n, start, dur, _reveal_cues(series, start)) if n else []
...
def dumbbell(sid, sc):
    ...
    times = _reveal_times(n, start, dur, _reveal_cues(items, start)) if n else []
    for i, it in enumerate(items):
        y = TY + i * rowh
...
BLOCKS = {"stat": stat_lockup, "statement": highlight_statement, "geo": geo_map, "raw": raw_scene,
          ...
          # --- Tier-2 (formerly compose_extension.EXT_BLOCKS) ---
          "spotlight": spotlight, "data_table": data_table,
