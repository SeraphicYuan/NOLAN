...
import { fitFontSize } from "../../primitives/fit";
...
type Line = { text: string; accent?: boolean; strike?: boolean; strikeWord?: string };
...
export type HeroStatementProps = {
  ...
  // passes this to every block; here it drives optional word-timed strikes.
  words?: Word[];
};
...
export const HeroStatement: React.FC<HeroStatementProps> = ({ kicker, lines, revealFrames, words = [] }) => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();

  // Auto-fit: shrink the lines uniformly so the longest fits the stage width
  // (capped at the design size); a long punchline no longer overflows or wraps.
  const heroSize = useMemo(
    () => fitFontSize(lines.map((l) => l.text), { withinWidth: 1640, fontVar: "--font-display-en", fontWeight: 900, max: 128, min: 44 }),
    [lines],
  );
      ...
      <div style={{ display: "flex", flexDirection: "column", justifyContent: "center", height: "100%" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "var(--space-4)" }}>
          {lines.map((ln, i) => {
            ...
            // Strike draws across after the line has settled. If the line names
            // a `strikeWord` that exists in the step's word timeline, sync the
            // draw to that word's spoken span (like NpcStrike); otherwise keep
            // the reveal-relative draw-on.
            const spoken = ln.strikeWord
              ? words.find((w) => norm(w.text) === norm(ln.strikeWord!))
              : undefined;
            ...
            return (
              <div key={i} style={{ overflow: "hidden", paddingBottom: "0.08em" }}>
                <div style={{
                  ...
                  fontSize: heroSize, lineHeight: 1.02,
