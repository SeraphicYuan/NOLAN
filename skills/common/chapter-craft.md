...
## 屏幕语言：跟随文章语言（强制 · On-screen language）

**屏幕上所有可见文字，必须使用文章本身的语言。** 该语言从 `script.md` / `source` 检测，并
写在 `outline.md` 头部（如 `On-screen language: English (en)`）。脚手架自带的 `01-example`
章节是中文，**仅作结构示意 —— 严禁继承它的语言**。

- 文章是英文 → 标题 / kicker / 标签 / 副文案 / mono cue / 列表项 / 结尾 **全部英文**。
- 文章是中文 → 全部中文。只有术语 / 代码 / 专有名词（Spiral Dynamics、API 名等）这种本就
  跨语言的词才允许保留原文。
- **完工自检必须核对**：本章屏幕文字语言 == 声明语言？

> 为什么单列一条：并行 subagent 各自照着 example 写，最容易在「语言」上漂（example 是中文
> → 整章悄悄变中文）。主题 token 只兜底视觉（颜色 / 字体），**兜不住语言** —— 必须显式约束。
