...
# 复用图形库（figures-first：StepFlow / HubSpoke / CompareCards / VersusPair / CardGrid /
# ProportionBar + primitives）。见 references/figures.md。优先用 figure，再 compose primitives，
# 最后才手写 Raw。
cp -r "$TEMPLATE/article/figures" "$TARGET/article/figures"
...
# WSL / DrvFs (/mnt/*) 上 npm 无法 chmod bin-links；失败则用 --no-bin-links 兜底。
if ! npm install >/dev/null 2>&1; then
  echo "  (bin-links 不可用 —— 用 --no-bin-links 重试)"
  npm install --no-bin-links >/dev/null 2>&1 || { echo "✗ npm install 失败" >&2; exit 1; }
fi
npm install reacticle@latest >/dev/null 2>&1 || npm install reacticle@latest --no-bin-links >/dev/null 2>&1 || true

# 没 bin-links 时手动补 .bin 软链，让 npm run dev/build 仍可用（DrvFs 上文件已可执行）
mkdir -p node_modules/.bin
[ -e node_modules/.bin/vite ] || ln -sf ../vite/bin/vite.js node_modules/.bin/vite 2>/dev/null || true
[ -e node_modules/.bin/tsc ]  || ln -sf ../typescript/bin/tsc  node_modules/.bin/tsc  2>/dev/null || true

# 默认英文 UI：把 reacticle 内置的中文标签（目录 / 复制 / 摘要…）替换为英文。
node "$SKILL_DIR/scripts/patch-reacticle-i18n.mjs" 2>/dev/null || true
...
echo "▸ 提示：visuals 优先用 figures（见 references/figures.md），离线字体可跑 scripts/embed-theme-fonts.py。"
