...
| Phase 2 Planning | `references/article-types.md` · `references/information-density.md` · `references/plan-template.md` · `references/theme-selection.md` · `references/layout.md` · `references/asset-policy.md` · `references/cover.md`（封面构图想法）· `references/figures.md`（视觉规划：每个 visual 在规划期解析成 figure / primitives / raw） | `references/article-types/<type>.md` · `theme-profiles/*.md` |
...
- **Colophon · 不可移除**：scaffold 在 `article/Article.tsx` 末尾自带 colophon Raw 块
  （`Made with NOLAN · <主题> theme`，低对比小字、theme token 自适应；NOLAN-native skill）。
