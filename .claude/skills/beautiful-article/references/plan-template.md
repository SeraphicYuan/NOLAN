...
1. <编号 NN> <标题>
   ...
   - **Visual（figure-first · 在规划期就定，见 `references/figures.md`）**：每个需要可视化的点
     直接解析成一个标识，**不要只写"需要 Raw：是"**：
       - `none` —— 正文即可，无需视觉。
       - `figure:<id>` —— 命中某个复用 figure。按意图解析：
         *顺序流程* → `StepFlow` · *一对多连接* → `HubSpoke` · *多项对比* → `CompareCards` ·
         *A/B 对照* → `VersusPair` · *卡片网格/清单* → `CardGrid` · *占比/容量* → `ProportionBar` ·
         *可折叠 Q&A* → `Detail`（reacticle 原生）。
       - `figure:mermaid` / `figure:chart` —— 按需 add-on（见 `references/registry.md`，用
         `scripts/add-figure.mjs` 安装）。*复杂结构图（flowchart/sequence/state/ER/gantt/timeline…）*
         → mermaid（慎用，~+3.7MB）；*真实数据图表（折线/柱/面积）* → chart。仅在核心 figure 不够时。
       - `primitives:<一句话组合>` —— figure 不完全贴合，但可由 `Card/Chip/Sticker/Arrow/...` 拼出。
       - `raw:<理由>` —— 仅当这是本文"招牌"视觉且上面都不贴合时；写清为何 figure 不适用。
     每个 visual 再补一句"服务哪个论点 / 目的"。**全篇 raw 数量应很少（理想 0-1 个）。**
2. ...
