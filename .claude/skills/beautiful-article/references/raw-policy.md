...
> **先看 `references/figures.md`。** Raw 是最后一档：先用 **figure**（StepFlow / HubSpoke /
> CompareCards / VersusPair / CardGrid / ProportionBar），再 **compose primitives**，**只有**
> 文章的"招牌"视觉才手写 Raw。绝大多数原本要手写的 Raw 其实是这几种复现的形状之一 —— 用 figure
> 既省 token、又一致、又跨主题。下面的规则适用于"确实需要手写 Raw"的那一两处。
