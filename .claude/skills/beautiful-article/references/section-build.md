...
读取：references/section-author-contract.md（组件 API + token 规则 + figure-first，一份就够）+
      references/figures.md（figure 目录：visuals 优先用 figure，再 compose primitives，最后才 Raw）+
      plan/plan.md 的 Outline 段本节段落 + Brief 段（信息保留比例）+
      source/source.md 本节对应内容 + 选定主题 theme-profiles/<id>.md +
      第一个 section 文件作为“代码风格”参考（不是抄袭对象）。
（有了 section-author-contract.md，prompt 不必再重复粘贴组件 API / token 规则 —— 让 subagent 读它即可。）
硬规则：
