...
3. **检测并声明屏幕语言**：从 `script.md` 检测文章语言，写进 `outline.md` 头部一行
   `On-screen language: <Name> (<code>)`。**所有章节的屏幕文字都用这个语言**（脚手架
   example 是中文，仅示意，禁止继承）—— 见 `CHAPTER-CRAFT.md`「屏幕语言」。并行开发时这是
   最容易漂的一项，必须显式传给每个 chapter agent。
